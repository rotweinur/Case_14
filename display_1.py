import pygame
from strings_1 import TRANSLATIONS

def init_display(rows, cols, cell_size=20, lang="ru"):
    """
    Initializes the pygame window and display configuration.

    Args:
        rows (int): Number of rows in the grid.
        cols (int): Number of columns in the grid.
        cell_size (int): Size of one cell in pixels.
        lang (str): Initial language code ('ru' or 'en').

    Returns:
        tuple: A tuple containing the screen surface and configuration dictionary.
    """
    pygame.init()
    pygame.font.init()

    width = cols * cell_size
    height = rows * cell_size + 40
    screen = pygame.display.set_mode((width, height))
    
    pygame.display.set_caption(TRANSLATIONS[lang]["title"])

    font = pygame.font.SysFont("Arial", 20)
    config = {
        "rows": rows,
        "cols": cols,
        "cell_size": cell_size,
        "font": font,
        "lang": lang,
        "translations": TRANSLATIONS,
        "colors": {
            "alive": (0, 200, 0),
            "dead": (30, 30, 30),
            "grid": (50, 50, 50),
            "bg": (10, 10, 10),
            "text": (255, 255, 255),
        }
    }
    return screen, config


def draw_grid(screen, grid, config):
    """
    Draws the grid based on the current state.

    Args:
        screen (pygame.Surface): The surface to draw on.
        grid (list[list[int]]): The 2D list representing the grid.
        config (dict): The display configuration dictionary.

    Returns:
        None
    """
    rows = config["rows"]
    cols = config["cols"]
    cell_size = config["cell_size"]
    colors = config["colors"]

    screen.fill(colors["bg"])
    for row in range(rows):
        for col in range(cols):
            color = colors["alive"] if grid[row][col] else colors["dead"]
            rect = pygame.Rect(col * cell_size, row * cell_size, cell_size, cell_size)
            pygame.draw.rect(screen, color, rect)
            pygame.draw.rect(screen, colors["grid"], rect, 1)


def draw_ui(screen, generation, speed, running, config):
    """
    Displays simulation information on the screen with localization.

    Args:
        screen (pygame.Surface): The surface to draw on.
        generation (int): Current generation number.
        speed (float): Current simulation delay.
        running (bool): Whether the simulation is active.
        config (dict): The display configuration dictionary.

    Returns:
        None
    """
    font = config["font"]
    colors = config["colors"]
    lang = config["lang"]
    texts = config["translations"][lang]
    
    y_offset = config["rows"] * config["cell_size"] + 5
    status = texts["running"] if running else texts["paused"]
    
    text = f"{texts['gen']}: {generation} | {texts['speed']}: {speed:.2f} | {status}"
    surface = font.render(text, True, colors["text"])
    screen.blit(surface, (10, y_offset))


def get_cell_from_mouse(pos, config):
    """
    Converts mouse position to grid cell indices.

    Args:
        pos (tuple[int, int]): Mouse position coordinates (x, y).
        config (dict): The display configuration dictionary.

    Returns:
        tuple[int, int] or None: Row and column indices or None if outside.
    """
    x, y = pos
    cell_size = config["cell_size"]
    row, col = y // cell_size, x // cell_size
    if 0 <= row < config["rows"] and 0 <= col < config["cols"]:
        return row, col
    return None